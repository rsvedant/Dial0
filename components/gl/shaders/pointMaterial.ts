  ...
  constructor() {
    super({
      ...
      void main() {
        ...
        // Black particles
        vec3 particleColor = vec3(0.0, 0.0, 0.0); // Pure black
        vec3 highlightColor = vec3(0.15, 0.15, 0.15); // Very dark gray for subtle highlights
        
        // Mix between base color and highlight based on sparkle brightness
        vec3 finalColor = mix(particleColor, highlightColor, (sparkleBrightness - 0.7) / 1.3);
        
        gl_FragColor = vec4(finalColor, mix(alpha * 0.65, (sparkleBrightness - 1.1) * 0.65, uTransition));
      }`,
      uniforms: {
